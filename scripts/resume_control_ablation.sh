...
# Create the remote script to resume E2E training from epoch 1800
sshpass -p "${PASS}" ssh -p ${PORT} -o StrictHostKeyChecking=no ${USER}@${HOST} "cat << 'REMOTE_EOF' > /root/autodl-tmp/hybrid_lens_design/scripts/resume_control_queue.sh
#!/usr/bin/env bash
set -euo pipefail
...
sshpass -p "${PASS}" ssh -p ${PORT} -o StrictHostKeyChecking=no ${USER}@${HOST} "chmod +x /root/autodl-tmp/hybrid_lens_design/scripts/resume_control_queue.sh"
sshpass -p "${PASS}" ssh -p ${PORT} -o StrictHostKeyChecking=no ${USER}@${HOST} "nohup bash /root/autodl-tmp/hybrid_lens_design/scripts/resume_control_queue.sh > /root/autodl-tmp/hybrid_lens_design/scripts/control_resume_launcher.log 2>&1 &"

echo "Remote control resume queue successfully injected and started!"
