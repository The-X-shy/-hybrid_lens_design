...
# 1. Create the remote control group script
sshpass -p "${PASS}" ssh -p ${PORT} -o StrictHostKeyChecking=no ${USER}@${HOST} "cat << 'REMOTE_EOF' > /root/autodl-tmp/hybrid_lens_design/scripts/run_control_queue.sh
#!/usr/bin/env bash
set -euo pipefail
...
\"\${PY}\" \"\${ROOT}/scripts/train_e2e.py\" \\
  ...
  --hybridlens \"\${OUT_DIR}/01_geolens/final_lens.json\" \\
  --epochs 5000 \\
  --freeze_doe 1 \\
...
sshpass -p "${PASS}" ssh -p ${PORT} -o StrictHostKeyChecking=no ${USER}@${HOST} "chmod +x /root/autodl-tmp/hybrid_lens_design/scripts/run_control_queue.sh"
sshpass -p "${PASS}" ssh -p ${PORT} -o StrictHostKeyChecking=no ${USER}@${HOST} "nohup bash /root/autodl-tmp/hybrid_lens_design/scripts/run_control_queue.sh > /root/autodl-tmp/hybrid_lens_design/scripts/control_launcher.log 2>&1 &"

echo "Remote control queue successfully injected and started!"
