...
echo "[INFO] Resuming E2E Training from epoch ${LAST_EPOCH}..." | tee -a "${RUN_LOG}"
"${PY}" "${ROOT}/scripts/train_e2e.py" \
  --config "${ROOT}/configs/exp_f2p8_80mm_e2e_fullsafe.yaml" \
  --hybridlens "${FULL_ROOT}/03_e2e/hybridlens_epoch${LAST_EPOCH}.json" \
  --pretrained "${FULL_ROOT}/03_e2e/network_epoch${LAST_EPOCH}.pth" \
...
"${PY}" "${ROOT}/scripts/evaluate_e2e.py" \
  ...
  --hybridlens "${FULL_ROOT}/03_e2e/hybridlens_epochfinal.json" \
  --output_dir "${FULL_ROOT}/03_e2e" >> "${RUN_LOG}" 2>&1
