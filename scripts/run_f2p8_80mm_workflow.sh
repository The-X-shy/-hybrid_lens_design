...
"${PYTHON_BIN}" scripts/train_geolens.py \
  ...
  --output_dir "${SMOKE_ROOT}/01_geolens" >> "${RUN_LOG}" 2>&1

echo "[INFO] Smoke Test: Hybrid" | tee -a "${RUN_LOG}"
"${PYTHON_BIN}" scripts/train_hybridlens.py \
  --config configs/exp_f2p8_80mm_hybrid.yaml \
  --geolens "${SMOKE_ROOT}/01_geolens/final_lens.json" \
  --iterations 100 \
  --wavelength 0.55 \
  --output_dir "${SMOKE_ROOT}/02_hybrid" >> "${RUN_LOG}" 2>&1

echo "[INFO] Smoke Test: E2E" | tee -a "${RUN_LOG}"
"${PYTHON_BIN}" scripts/train_e2e.py \
  --config configs/exp_f2p8_80mm_e2e.yaml \
  --hybridlens "${SMOKE_ROOT}/02_hybrid/hybrid_final.json" \
  --epochs 100 \
  --freeze_doe 1 \
  --network_type mha_unet \
  --output_dir "${SMOKE_ROOT}/03_e2e" >> "${RUN_LOG}" 2>&1
...
"${PYTHON_BIN}" scripts/train_metalens.py \
  ...
  --geolens "${SMOKE_ROOT}/01_geolens/final_lens.json" \
  --iterations 100 \
  --output_dir "${SMOKE_ROOT}/04_meta" >> "${RUN_LOG}" 2>&1
...
"${PYTHON_BIN}" scripts/train_geolens.py \
  ...
  --output_dir "${FULL_ROOT}/01_geolens" >> "${RUN_LOG}" 2>&1
...
"${PYTHON_BIN}" scripts/train_hybridlens.py \
  ...
  --iterations 5000 \
  --wavelength 0.55 \
  --output_dir "${FULL_ROOT}/02_hybrid" >> "${RUN_LOG}" 2>&1
...
"${PYTHON_BIN}" scripts/generate_training_log.py \
  --experiment_name "f2p8_80mm_${TS}" \
  --base_dir "${FULL_ROOT}" \
  --smoke_dir "${SMOKE_ROOT}" \
  --output "${LOG_ROOT}/training_log_f2p8_80mm_${TS}.md" >> "${RUN_LOG}" 2>&1
