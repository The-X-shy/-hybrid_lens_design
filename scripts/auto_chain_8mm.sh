...
sshpass -p "${PASS}" ssh -p ${PORT} -o StrictHostKeyChecking=no ${USER}@${HOST} "cat << 'REMOTE_EOF' > /root/autodl-tmp/hybrid_lens_design/scripts/run_hybrid_and_e2e.sh
#!/usr/bin/env bash
set -euo pipefail
...
sshpass -p "${PASS}" ssh -p ${PORT} -o StrictHostKeyChecking=no ${USER}@${HOST} "chmod +x /root/autodl-tmp/hybrid_lens_design/scripts/run_hybrid_and_e2e.sh"
sshpass -p "${PASS}" ssh -p ${PORT} -o StrictHostKeyChecking=no ${USER}@${HOST} "nohup bash /root/autodl-tmp/hybrid_lens_design/scripts/run_hybrid_and_e2e.sh > /root/autodl-tmp/hybrid_lens_design/scripts/launcher.log 2>&1 &"

echo "Auto-chain script successfully injected and started on the remote server!"
